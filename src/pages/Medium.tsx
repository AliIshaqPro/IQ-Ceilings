  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        ...
        const premiumPlan = response.data.find(plan => plan.title === "Premium");
        if (premiumPlan && premiumPlan.gallery) {
          setImages(premiumPlan.gallery.map(img => img.url));
          setTitle(premiumPlan.title);
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-dark relative overflow-x-hidden">
      ...
      <div className="relative container mx-auto px-4 sm:px-6 py-24 w-full">
        <div className="text-center mb-16 animate-fade-in">
          <h1 className="font-display text-4xl sm:text-6xl md:text-7xl font-bold text-foreground mb-4">
            {title}
          </h1>
          <div className="flex items-center justify-center gap-3">
            <div className="h-px w-16 bg-gradient-to-r from-transparent to-accent" />
            <p className="text-muted-foreground text-sm tracking-widest uppercase">100 Rs per sq. ft.</p>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 w-full">
            ...
            <div
              key={index}
              className="group relative overflow-hidden rounded-xl bg-card border border-border transition-all duration-500 hover:border-accent/50 hover:shadow-glow animate-scale-in aspect-[4/3] cursor-pointer"
