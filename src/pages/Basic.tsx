...
const Basic = () => {
  ...
  const [title, setTitle] = useState("Essential");

  useEffect(() => {
    const loadImages = async () => {
      try {
        const response = await fetchPricingPlans();
        const essentialPlan = response.data.find(plan => plan.title === "Essential");
        if (essentialPlan && essentialPlan.gallery) {
          setImages(essentialPlan.gallery.map(img => img.url));
          setTitle(essentialPlan.title);
        }
      } catch (error) {
      ...
      <div className="relative container mx-auto px-4 sm:px-6 py-24 w-full">
        <div className="text-center mb-16 animate-fade-in">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <p className="text-muted-foreground text-sm tracking-widest uppercase">90 Rs per sq. ft.</p>
